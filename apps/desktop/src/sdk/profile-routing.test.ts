...
const { $activeGatewayProfile, $gatewaySwapTarget, $profiles, ensureGatewayAgent, ensureGatewayProfile, refreshProfiles } =
...
describe('profile-aware plugin session opens', () => {
  it('captures the full owner route before opening a remote session', async () => {
    ...
    expect(ensureGatewayAgent).toHaveBeenCalledWith('source-a', 'default')
