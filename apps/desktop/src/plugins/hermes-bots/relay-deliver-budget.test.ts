...
// Fork note: the backend sources live outside the renderer's split paths and
// don't exist in this fork — the two mirror tests below skip here (they still
// run upstream); the pure-TS deadline test always runs.
const configDefaultsPath = join(repoRoot, 'hermes_cli/config_defaults.py')
const relayHandlerPath = join(repoRoot, 'tui_gateway/methods_bot_relay.py')
const backendSourcesPresent = existsSync(configDefaultsPath) && existsSync(relayHandlerPath)
const configDefaults = backendSourcesPresent ? readFileSync(configDefaultsPath, 'utf8') : ''
const relayHandler = backendSourcesPresent ? readFileSync(relayHandlerPath, 'utf8') : ''
