...
describe('ModelMenuPanel provider collapse', () => {
  ...
  it('keeps the current pick when Refresh Models no longer lists it', async () => {
    ...
    getGlobalModelOptions
      // Fork note: the panel's mount auto-refresh (a655324) consumes extra
      // catalog reads — the first three mocks serve mount (catalog query +
      // silent refresh), the fourth serves the explicit click.
      ...
      // Mount now issues three reads (catalog query + silent auto-refresh);
      // the explicit click below is the fourth.
      .mockResolvedValueOnce({
        model: 'glm-4.5-air',
        provider: 'zhipu',
        providers: [DEEPSEEK_PROVIDER, MOA_PROVIDER]
    ...
    await vi.waitFor(() => {
      expect(getGlobalModelOptions).toHaveBeenCalledTimes(3)
    })
    expect(onSelectModel).not.toHaveBeenCalled()
  ...
  it('does not switch when Refresh Models still lists the current pick', async () => {
    ...
    // Three catalog reads by now: the mount query, the mount auto-refresh
    // effect (a655324 — silent refresh:true on every open), and the manual
    // refresh above.
    await vi.waitFor(() => {
      expect(getGlobalModelOptions).toHaveBeenCalledTimes(3)
    })
  ...
  it('does not rewrite the provider when Refresh Models lists the same model id elsewhere', async () => {
    ...
    getGlobalModelOptions
      // Fork note: the panel's mount auto-refresh (a655324) consumes extra
      // catalog reads — the first three mocks serve mount (catalog query +
      // silent refresh), the fourth (the drop) serves the explicit click.
      .mockResolvedValueOnce({
