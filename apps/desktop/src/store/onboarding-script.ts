...
export function buildChatOnboardingPrompt(suggestedName?: string | null, signedIn = false, capabilities = ''): string {
  ...
  return [
    ...
    'If they type a custom colour instead of choosing a swatch, address it before moving on. A typed colour does not apply itself. Use only an available control or tool and confirm its result; never invent a directive attribute. If unsupported or failed, name the requested colour, say it was not applied and why (or that the cause is unknown), and keep the existing colour. Point back to supported swatches in the existing picker, not a silent substitute or another copy of the look card. Stay on this choice until they accept a colour, keep the current one, or ask to skip; an unsuccessful custom request does not complete the colour step.',
