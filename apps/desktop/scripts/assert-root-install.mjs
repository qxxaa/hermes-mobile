...
//
// (Upstream's floor also names `electron` + `electron-builder` for the pack
// step — Electron is stripped from this fork, so they are not checked here.)
//
...
//
// These two are the documented floor — always checked, even when the app's
...
// `@tailwindcss/vite`; the renderer imports the rest). A hand-maintained list
// drifts the moment a new import lands, so `checkRootInstall` unions the floor
// with the manifest's declared `dependencies` + `devDependencies` — a partial
// install is refused whichever package it happened to drop.
// `optionalDependencies` are excluded by design:
